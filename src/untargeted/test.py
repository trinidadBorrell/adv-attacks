...
Usage:
    python test.py <test_type> <original_image_path> <untargeted_image> <targeted_image> <original_fine_class> <original_coarse_class> <epsilon>

    test_type: 1 or 2 (type of test to perform)
    original_image_path: Path to original image
    untargeted_image_path: Path to untargeted adversarial image
    targeted_image_path: Path to targeted adversarial image
    original_fine_class: ImageNet class ID (0-999)
    original_coarse_class: Coarse class label or None
...
class AdversarialTester:
    ...
    def test_1_top_category_check(
        ...
        untargeted_top_indices, untargeted_top_probs = self.get_top_predictions(
            untargeted_logits
        )
        ...
        untargeted_success = untargeted_top_prediction not in original_coarse_indices

        # Check targeted: TOP (first) category should contain original coarse category
        targeted_top_prediction = targeted_top_indices[0]
        targeted_success = targeted_top_prediction in original_coarse_indices

        overall_success = untargeted_success and targeted_success

        results = {
            "test_type": 1,
            "untargeted_top_indices": untargeted_top_indices,
            "untargeted_top_probs": untargeted_top_probs,
            "targeted_top_indices": targeted_top_indices,
            "targeted_top_probs": targeted_top_probs,
            "untargeted_top_prediction": untargeted_top_prediction,
            "targeted_top_prediction": targeted_top_prediction,
            "original_coarse_indices": original_coarse_indices,
        }

        logger.info(
            f"Test 1 Results: Overall={overall_success}, Untargeted={untargeted_success}, Targeted={targeted_success}"
        )
    ...
    def test_2_coarse_score_check(
        ...
        if original_coarse_class not in self.coarse_labels:
            logger.warning(
                f"Original coarse class '{original_coarse_class}' not found in coarse labels. Skipping Test 2."
            )
            return False, {
                "test_type": 2,
        ...
        untargeted_coarse_score = compute_coarse_score(
            untargeted_logits, coarse_class_idx, self.coarse_indices
        ).item()
        targeted_coarse_score = compute_coarse_score(
            targeted_logits, coarse_class_idx, self.coarse_indices
        ).item()

        logger.info(f"Untargeted coarse score: {untargeted_coarse_score}")
        logger.info(f"Targeted coarse score: {targeted_coarse_score}")

        # Check conditions
        untargeted_success = untargeted_coarse_score < 0
        ...
        results = {
            "test_type": 2,
            "untargeted_coarse_score (Sc)": untargeted_coarse_score,
            ...
            "untargeted_top_prediction": untargeted_top_prediction,
            "targeted_top_prediction": targeted_top_prediction,
            "original_coarse_indices": coarse_class_idx,
        }

        logger.info(
            f"Test 2 Results: Overall={overall_success}, Untargeted={untargeted_success}, Targeted={targeted_success}"
        )
    ...
    def save_successful_results(
        ...
        self._save_tensor_as_image(untargeted_image, attack_folder / "untargeted.png")
        ...
        if test_type == 1:
            untargeted_top_indices = test_results["untargeted_top_indices"]
            untargeted_top_probs = test_results["untargeted_top_probs"]
        ...
        untargeted_class_probs = {
            class_names.get(idx, f"class_{idx}"): prob
            for idx, prob in zip(untargeted_top_indices, untargeted_top_probs)
        }
        ...
        untargeted_output = {
            "logits": untargeted_logits.cpu().numpy().tolist(),
            "class_probabilities": untargeted_class_probs,
            "top_indices": untargeted_top_indices,
            "top_probs": untargeted_top_probs,
        }
        ...
        if test_type == 1:
            untargeted_output.update(
                {
                    "top_prediction": test_results["untargeted_top_prediction"],
            ...
            untargeted_output.update(
                {
                    "coarse_score_Sc": test_results["untargeted_coarse_score (Sc)"],
                    "top_prediction": test_results["untargeted_top_prediction"],
                    "original_coarse_indices": test_results["original_coarse_indices"],
                }
            )
        ...
        with open(attack_folder / "untargeted_ensemble_output.json", "w") as f:
            json.dump(untargeted_output, f, indent=2)
...
def main():
    ...
    if len(sys.argv) != 8:
        print(
            "Usage: python test.py <test_type> <original_image_path> <untargeted_image_path> <targeted_image_path> <original_fine_class> <original_coarse_class> <epsilon>"
        )
