...
def _process_single_attack(args: tuple[str, int, str, float, int, Path, str]) -> tuple[bool, dict, str, dict]:
    ...
    """
    image_path, fine_class_id, coarse_class, epsilon, test_type, output_base_dir, device = args
...
def _process_image_all_combinations(args: tuple[str, int, str, list, list, Path, str]) -> tuple[bool, dict, str, list]:
    ...
    """
    image_path, fine_class_id, coarse_class, epsilons, test_types, output_base_dir, device = args
    ...
    for epsilon in epsilons:
        for test_type in test_types:
            try:
                ...
                if not success and "validation_results" in results and not results.get("overall_success", True):
                ...
                logger.error(f"Error processing {Path(image_path).name} with eps={epsilon}, test={test_type}: {e}")
                metadata = {
                    "image_path": image_path,
                    "fine_class_id": fine_class_id,
                    "coarse_class": coarse_class,
...
def run_batch_attacks(
    ...
    for i, image_path in enumerate(image_paths):
        unique_image_tasks.append((
            image_path, fine_class_ids[i], coarse_classes[i], 
            epsilons, test_types, output_base_dir, device
        ))
    ...
    logger.info(f"Processing {total_images} unique images with {total_tasks} total tasks, batch_size={batch_size}")
    
    if batch_size == 1:
        # Sequential processing (original behavior)
        for i, task_args in enumerate(unique_image_tasks):
            logger.info(f"Processing image {i+1}/{total_images}: {Path(task_args[0]).name}")
            success, results, output_folder, metadata_list = _process_image_all_combinations(task_args)
        ...
        logger.info(f"Using {num_processes} parallel processes for {total_images} unique images")
        ...
        with Pool(processes=num_processes) as pool:
            ...
            # Process results
            for i, (image_success, image_results, image_output_folder, metadata_list) in enumerate(results):
    ...
    logger.info(f"Batch processing complete: {successful_attacks}/{total_tasks} successful attacks")
