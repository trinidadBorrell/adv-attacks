...
def main():
    ...
    try:
        ...
        # Generate 3 targeted attacks using top 3 categories
        logger.info(f"Generating 3 targeted attacks towards {target_coarse_class}...")

        targeted_images = generator.generate_targeted_attacks_top3(
            image, target_coarse_class, epsilon
        )

        # Generate control images (same magnitude of perturbation as targeted adversarial images)
        logger.info(
            "Generating control images with same magnitude of perturbation as targeted adversarial images..."
        )

        control_images = []
        for i, targeted_image in enumerate(targeted_images):
            control_image = generator.generate_control_image_from_targeted_attack(
                image, targeted_image, epsilon
            )
            control_images.append(control_image)
            logger.info(f"Generated control image {i + 1}/3")
