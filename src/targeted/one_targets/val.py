...
Usage:
    python val.py <original_image_path> <original_fine_class> <targeted_coarse_class>
...
class ImageValidator:
    ...
    def test_1_top_category_check(
        self, original_logits: torch.Tensor, target_coarse_class: str
        ...
        target_coarse_indices = self.coarse_indices[
            self.coarse_labels.index(target_coarse_class)
        ]
        ...
        success = original_top_prediction not in target_coarse_indices
        ...
        results = {
            ...
            "target_coarse_indices": target_coarse_indices,
            "success": success,
        }
    ...
    def test_2_coarse_score_check(
        ...
        if target_coarse_class not in self.coarse_labels:
            logger.warning(
                f"Target coarse class '{target_coarse_class}' not found in coarse labels. Test 2 failed."
            )
            return False, {
                "test_type": 2,
                "success": False,
        ...
        coarse_class_idx = self.coarse_labels.index(target_coarse_class)
        ...
        target_coarse_score = compute_coarse_score(
            original_logits, coarse_class_idx, self.coarse_indices
        ).item()

        logger.info(f"Target coarse score: {target_coarse_score}")

        # Check condition: coarse score < 0
        success = target_coarse_score < 0

        results = {
            "test_type": 2,
            "target_coarse_score_Sc": target_coarse_score,
            "original_top_indices": original_top_indices,
            "original_top_probs": original_top_probs,
            "original_top_prediction": original_top_prediction,
            "target_coarse_class": target_coarse_class,
            "success": success,
        }

        logger.info(
            f"Test 2 Results: Success={success}, Coarse score: {target_coarse_score}"
        )
    ...
    def validate_image(
        ...
        if test_type == 1:
            test_success, test_results = self.test_1_top_category_check(
                original_logits, target_coarse_class
            )
        elif test_type == 2:
...
def main():
    ...
    try:
        ...
        success, results = validator.validate_image(
            original_image_path, original_fine_class, target_coarse_class
        )
